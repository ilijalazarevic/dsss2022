...
data_set %>%
  ...
  summarise(mean_new_cases = mean(new_cases, na.rm=TRUE)) %>%
...
country_analysis_frame <- data_set %>% 
  filter(!grepl(data_set$iso_code, pattern="^OWID_")) %>%
  select(date, location, people_vaccinated_per_hundred, people_vaccinated, population, population_density, gdp_per_capita, total_cases) %>%
